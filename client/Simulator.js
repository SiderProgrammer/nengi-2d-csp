...
class Simulator {
  ...
  createEntity(entity) {
    ...
    if (entity.protocol.name === "Bonus") {
      const obs = new Bonus(entity.x, entity.y, entity.width, entity.height);
      this.bonuses.set(entity.nid, obs);
      this.renderer.createEntity(entity);
    }
  }
  ...
  processLocalMessage(message) {
    ...
    } else if (message.protocol.name === "BonusPickedUp") {
      this.bonuses.forEach((bonus) => {
        if (bonus.x === message.x && bonus.y === message.y) {
          this.renderer.deleteEntity(bonus.nid);
